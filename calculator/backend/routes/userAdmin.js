...
router.get('/:username/permissions', authMiddleware(['master']), async (req, res) => {
  try {
    ...
    const permissions = permissionRows
      ...
      .filter((row) => row.key && EFFECTS.has(row.effect));
...
router.put('/:username/permissions', authMiddleware(['master']), requireRecentAuth(), async (req, res) => {
  try {
    ...
    for (const row of rawPermissions) {
      ...
      if (!key || !EFFECTS.has(effect)) continue;
      permissionMap.set(key, effect);
    }

    const categorySetNames = Array.from(
      new Set(
        rawCategoryAccess
          .map((row) => normalizeKey(row?.courseConfigSetName))
          .filter(Boolean)
      )
    );
    ...
    for (const row of rawCategoryAccess) {
      ...
      const effect = String(row?.effect || '').trim();
      if (!setName || !categoryKey || !EFFECTS.has(effect)) continue;
    ...
    const userPermissionRows = keys.map((key) => ({
      userId: user.id,
      permissionId: idByKey.get(key),
      effect: permissionMap.get(key),
    }));
