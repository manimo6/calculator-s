...
type NoticeRow = { targets?: string[] } & Record<string, unknown>

const router = express.Router();

async function updateNotice(id: string, updater: Record<string, unknown>) {
  try {
...
router.get('/', authMiddleware(), async (req, res) => {
  try {
    ...
    const notices: NoticeRow[] = await prisma.notice.findMany({
      orderBy: { createdAt: 'desc' },
    });
    const filtered = notices.filter((n: NoticeRow) => {
