...
type CourseConfigRow = { name: string; data?: Record<string, unknown> }
type CourseTreeGroup = { cat?: string }
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const rows: CourseConfigRow[] = await prisma.courseConfigSet.findMany({
      ...
      const courseConfigSets: Record<string, unknown> = {};
      for (const row of rows) {
        const access = getAccessForSet(accessMap, row.name, bypassCategoryAccess);
        courseConfigSets[row.name] = filterCourseConfigSetData(
...
router.post('/', authMiddleware(), requirePermissions('tabs.courses'), async (req, res) => {
  try {
    ...
    if (!access.hasRules && !access.bypass) {
      return res.status(403).json({
        status: 'fail',
        message: 'No category access rules for this config set.',
      });
    }
    ...
    if (access.hasRules) {
      ...
      const tree: CourseTreeGroup[] = Array.isArray(data?.courseTree)
        ? data.courseTree
        : [];
    ...
    const tree: CourseTreeGroup[] = Array.isArray(nextData?.courseTree)
      ? nextData.courseTree
      : [];
