...
function normalizeCourseNote(note: Record<string, unknown>) {
  ...
  return {
    ...
    updatedAt: note.updatedAt ? new Date(String(note.updatedAt)) : new Date(),
  };
}
...
async function main() {
    ...
    ? users
        .filter(
          (u: Record<string, unknown>) =>
            Boolean(u && u.username && u.role && u.salt && u.hash)
        )
  ...
  const dbUsers: Array<{ id: string; username: string; role: string }> =
    await prisma.user.findMany({
    select: { id: true, username: true, role: true },
  });
  const userByUsername = new Map(
    dbUsers.map((u: { id: string; username: string; role: string }) => [u.username, u])
  );
  const adminUsers = dbUsers.filter((u: { role: string }) => u.role === 'admin');
