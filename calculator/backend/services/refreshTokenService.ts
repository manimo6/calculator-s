...
async function revokeRefreshTokenById(id: string, tx = prisma) {
  if (!id) return null;
...
async function revokeAllRefreshTokensForUser(userId: string, tx = prisma) {
...
async function rotateRefreshToken(token: string) {
  ...
  return prisma.$transaction(async (tx: import('@prisma/client').Prisma.TransactionClient) => {
    const issued = await issueRefreshToken(record.user, tx, reauthAt);
