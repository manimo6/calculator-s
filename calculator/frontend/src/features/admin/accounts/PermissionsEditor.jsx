...
function EffectChooser({ value, onChange, idPrefix, disabled }) {
...
function PermissionRow({ title, subtitle, value, onChange, idPrefix, disabled }) {
  return (
...
export default function PermissionsEditor({
  ...
  function renderPermissionGroup(list) {
    if (!list.length) {
      return (
    ...
    return (
      <div className="space-y-3">
        {list.map((perm) => {
          ...
          const label = PERMISSION_LABELS[key] || description || key
