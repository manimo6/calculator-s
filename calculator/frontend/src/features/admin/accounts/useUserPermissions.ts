...
type PermissionState = Record<string, PermissionEffect>
type CategoryState = Record<string, Record<string, PermissionEffect>>
type CourseConfigSet = { name?: string } & Record<string, unknown>
...
function buildPermissionState(rows: PermissionRow[] | null | undefined): PermissionState {
...
function buildCategoryState(rows: CategoryRow[] | null | undefined): CategoryState {
  const map: CategoryState = {}
  for (const row of rows || []) {
...
function toPermissionPayload(permissionState: PermissionState) {
  return Object.entries(permissionState || {})
    .filter(([, effect]) => effect === EFFECT_ALLOW || effect === EFFECT_DENY)
    .map(([key, effect]) => ({ key, effect }))
}

function toCategoryPayload(categoryState: CategoryState) {
  const out = []
...
export function useUserPermissions(username?: string) {
  ...
  const loadPermissions = useCallback(async () => {
    try {
      ...
      setPermissions([])
      const message = err instanceof Error ? err.message : "권한 목록을 불러오지 못했습니다."
      setError(message)
    }
  }, [])
  ...
  const loadUserPermissions = useCallback(async () => {
    ...
    try {
      ...
      setPermissionState({})
      setCategoryState({})
      const message = err instanceof Error ? err.message : "사용자 권한을 불러오지 못했습니다."
      setError(message)
    } finally {
      setLoading(false)
    }
  ...
  const savePermissions = useCallback(
    async (nextPermissionState: PermissionState, nextCategoryState: CategoryState) => {
      if (!username) return
      setSaving(true)
      setError("")
      try {
  ...
  const setPermissionEffect = useCallback(
    async (key: string, effect: PermissionEffect) => {
  ...
  const setPermissionEffects = useCallback(
    async (updates: Record<string, PermissionEffect> = {}) => {
      const next = { ...permissionState }
  ...
  const setCategoryEffect = useCallback(
    async (setName: string, categoryKey: string, effect: PermissionEffect) => {
      const normalized = normalizeEffect(effect)
  ...
  const setAllCategoriesEffect = useCallback(
    async (setName: string, nextEffect: PermissionEffect, categoryList: string[]) => {
