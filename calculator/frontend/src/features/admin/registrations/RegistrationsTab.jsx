...
function makeCourseValue(courseId, courseName) {
...
export default function RegistrationsTab({ user }) {
  ...
  const ganttTabsScrollRef = useRef(null)
  ...
  const transferCourseGroups = useMemo(() => {
    ...
    const addToGroup = (category, option) => {
  ...
  const handleCreateExtension = useCallback(
    async (payload) => {
      try {
        ...
        const ids = (baseRegistrations || []).map((r) => r?.id).filter(Boolean)
        if (ids.length) {
          await loadExtensions(ids)
        }
      } catch (err) {
        setError(err?.message || "연장 저장에 실패했습니다.")
        throw err
      }
    },
  ...
  const openWithdrawDialog = useCallback((registration) => {
    if (!registration) return
    const today = formatDateYmd(new Date())
    const defaultDate = registration?.withdrawnAt || today
  ...
  const openTransferDialog = useCallback((registration) => {
    if (!registration) return
    const today = formatDateYmd(new Date())
  ...
  const openNoteDialog = useCallback((registration) => {
    if (!registration) return
  ...
  const handleWithdrawSave = useCallback(async () => {
    ...
    setWithdrawError("")
    try {
      await apiClient.updateRegistrationWithdrawal(withdrawTarget.id, withdrawDate)
      await loadRegistrations()
      setWithdrawDialogOpen(false)
      setWithdrawTarget(null)
    } catch (err) {
      setWithdrawError(err?.message || "퇴원 처리에 실패했습니다.")
    } finally {
  ...
  const handleTransferSave = useCallback(async () => {
    ...
    const courseLabel = transferCourseLabelMap.get(transferCourseValue)
    if (!courseLabel) {
      setTransferError("전반 과목을 선택해 주세요.")
      return
    }
    ...
    try {
      ...
      setTransferError(err?.message || "전반 처리에 실패했습니다.")
    } finally {
  ...
  const handleRestore = useCallback(async (registration) => {
    if (!registration?.id) return
    const name = registration?.name || "학생"
    if (!window.confirm(`${name}의 퇴원 상태를 복구할까요?`)) return

    try {
      await apiClient.updateRegistrationWithdrawal(registration.id, null)
      await loadRegistrations()
    } catch (err) {
  ...
  const handleNoteSave = useCallback(async () => {
    ...
    setNoteError("")
    try {
      await apiClient.updateRegistrationNote(noteTarget.id, noteValue)
      await loadRegistrations()
      setNoteDialogOpen(false)
      setNoteTarget(null)
      setNoteValue("")
    } catch (err) {
      setNoteError(err?.message || "메모 저장에 실패했습니다.")
    } finally {
      setNoteSaving(false)
    }
  ...
  const handleTransferCancel = useCallback(
    async (registration) => {
      ...
      try {
        await apiClient.cancelTransferRegistration(registration.id)
        await loadRegistrations()
      } catch (err) {
        setError(err?.message || "전반 취소에 실패했습니다.")
      }
    },
    [loadRegistrations, setError]
  )

  const handleGanttTabsWheel = useCallback((event) => {
  ...
  useEffect(() => {
    ...
    const handler = (event) => handleGanttTabsWheel(event)
  ...
  const ganttGroups = useMemo(() => {
    ...
    const normalizeCourse = (value) => String(value || "").trim()
    const matchesCourse = (courseName, target) => {
    ...
    const collectCourseDays = (courseNames) => {
    ...
    const getCourseLabel = (key, fallback) => {
                ...
                >
                  ...
                  >
                    <Calendar
                      ...
                      onSelect={(date) => {
                        setWithdrawDate(date ? formatDateYmd(date) : "")
                        setWithdrawPickerOpen(false)
                      }}
                      initialFocus
                    />
            ...
            <div className="space-y-4 text-sm">
                ...
                >
                  ...
                  >
                    <Calendar
                      ...
                      onSelect={(date) => {
                        setTransferDate(date ? formatDateYmd(date) : "")
                        setTransferPickerOpen(false)
                      }}
                      initialFocus
                    />
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="예: 8"
                  value={transferWeeks}
                  onChange={(event) => setTransferWeeks(event.target.value)}
                />
              </div>
            </div>
          ) : null}
