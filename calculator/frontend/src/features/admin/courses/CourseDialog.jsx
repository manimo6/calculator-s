...
function DayCheckboxes({ field, state, dispatch }) {
  return (
...
function SingleDayCheckboxes({ field, state, dispatch }) {
  return (
    <div className="flex flex-wrap gap-2">
      {[1, 2, 3, 4, 5, 6, 0].map((d) => {
        const checked = state[field].includes(d)
...
export default function CourseDialog({
  ...
  const [breakPickerOpen, setBreakPickerOpen] = useState(null)

  const handleTextbookOptionChange = (optionKey, amountKey) => (value) => {
            ...
            <CardContent className="space-y-4">
              <div className="space-y-3">
                ...
                <RadioGroup
                  ...
                  onValueChange={(value) =>
                    dispatch({ type: "SET_FIELD", field: "timeType", value })
                  }
