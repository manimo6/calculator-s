...
type CourseConfigSetToolbarProps = {
  courseConfigSetList: Array<{ name?: string }>
  selectedCourseConfigSet: string
  onCourseConfigSetChange: (value: string) => void
  storageScope?: string
  canDelete?: boolean
  actionsDisabled?: boolean
  onSaveCourseConfigSet: () => void
  onOverwriteCourseConfigSet: () => void
  onDeleteCourseConfigSet: () => void
}

export default function CourseConfigSetToolbar(props: CourseConfigSetToolbarProps) {
  const {
    courseConfigSetList,
    ...
    <Card className="border-border/60 bg-card/60">
      ...
      <CardContent className="flex flex-wrap items-end gap-3">
        <CourseConfigSetPicker
          ...
          courseConfigSetList={courseConfigSetList.map((item) => item.name || "").filter(Boolean)}
