...
type ReauthAction<T> = () => Promise<T> | T
type ReauthResult<T> = { ok: true; value: T } | { ok: false }
...
function isReauthError(error: unknown) {
  return (
    ...
    error !== null &&
    "message" in error &&
    String((error as { message?: string }).message || "") === REAUTH_ERROR_MESSAGE
  )
}
...
export function useReauthDialog() {
  ...
  const resolvingRef = useRef<((value: boolean) => void) | null>(null)
  ...
  const confirmReauth = useCallback(async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "재인증에 실패했습니다."
      setError(message)
    }
  }, [password, resetState])

  const withReauth = useCallback(
    async <T,>(action: ReauthAction<T>): Promise<ReauthResult<T>> => {
