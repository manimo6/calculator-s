...
import { fetchCourseData, courseTree, courseInfo, timeTable, recordingAvailable, weekdayName, courseToCatMap } from '../../utils/data';
...
const CourseManager = () => {
    ...
    // Modals
    type CourseModalDynamicOption = { label: string; time: string }
    type CourseModalTimeData =
        | string
        | { type: 'onoff'; online?: string; offline?: string }
        | { type: 'dynamic'; options?: CourseModalDynamicOption[] }
        | Record<string, string>
    type CourseModalRecording = boolean | { 온라인?: boolean; 오프라인?: boolean }

    type CourseModalData = {
        category?: string
        name?: string
        info?: Record<string, unknown>
        timeData?: CourseModalTimeData
        recording?: CourseModalRecording
    }

    type ModalProps = {
        editingCourseId?: string
        courseData?: CourseModalData
        editingCategory?: string
    }

    const [modal, setModal] = useState<{ type: string | null; props?: ModalProps }>({ type: null, props: {} });
    ...
    const handleSaveCategory = (newName: string, originalName?: string) => {
    ...
    type CourseFormData = {
        ...
        timeOffline?: string
        timeDefault?: string
        dynamicOptions?: Array<{ label?: string; time?: string }>
        ...
        mathExcludedFee?: number | string
    }

    const handleSaveCourse = (formData: CourseFormData, courseId?: string) => {
        ...
        const buildCourseInfo = (key: string, prev?: Record<string, unknown> & { endDay?: number }) => {
            ...
            const fallbackEndDay = typeof prev?.endDay === 'number' ? prev.endDay : 5;
            next.endDay = next.endDays?.length ? next.endDays[0] : fallbackEndDay;
    ...
    const handleSaveCourseConfigSet = async () => {
        ...
        try {
            ...
            const message = e instanceof Error ? e.message : '설정 세트 저장 실패';
            showToast(message);
        }
    };
    ...
    const handleLoadCourseConfigSet = async () => {
        ...
        try {
            ...
            const message = e instanceof Error ? e.message : '설정 세트 불러오기 실패';
            showToast(message);
        }
    };
    ...
    const handleDeleteCourseConfigSet = async () => {
        ...
        try {
            ...
            const message = e instanceof Error ? e.message : '설정 세트 삭제 실패';
            showToast(message);
        }
    };
    ...
    return (
        <div className="course-manager">
            ...
            <CourseModal
                ...
                editingCourseId={modal.props?.editingCourseId}
                courseData={modal.props?.courseData}
                {...modal.props}
            />
