...
import { weekdayName, type CourseTreeGroup } from '../../utils/data';
...
type CourseFormAction =
    ...
    | { type: 'TOGGLE_DAY'; field: 'days' | 'startDays' | 'endDays'; value: number }
...
type CourseModalProps = {
    isOpen: boolean
    onClose: () => void
    onSave: (formData: CourseFormState, courseId?: string) => void
...
function courseFormReducer(state: CourseFormState, action: CourseFormAction) {
    switch (action.type) {
...
const CourseModal = ({ isOpen, onClose, onSave, categories, editingCourseId, courseData }: CourseModalProps) => {
        ...
        if (isOpen && categories.length > 0) {
            if (editingCourseId && courseData) {
                ...
                if (!timeData || typeof timeData === 'string') {
                    ...
                    const options = Array.isArray(timeData.options) ? timeData.options : [];
                    tType = 'dynamic'; tDynamic = options;
                    ...
                    if (hasOnOff) {
                        ...
                        tType = 'dynamic';
                        tDynamic = keys.map((k) => ({ label: k, time: timeMap[k] })).filter((o) => o.label);
                    }
                }
    ...
    const DayCheckboxes = ({ field, type = 'checkbox' }: { field: 'days' | 'startDays' | 'endDays'; type?: 'checkbox' | 'radio' }) => (
