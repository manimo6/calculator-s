...
const NoticesTab = ({ user }: { user: AuthUser | null }) => {
  ...
  async function loadNotices() {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : '공지사항을 불러오지 못했습니다.';
      setError(message);
      setNotices([]);
    } finally {
      setLoading(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const message = e2 instanceof Error ? e2.message : '저장에 실패했습니다.';
      setError(message);
    }
  }
