...
type CourseTreeItem = { label?: string };
type CourseTreeGroup = { cat?: string; items?: CourseTreeItem[] };
type CourseConfigSet = { name: string; data?: { courseTree?: CourseTreeGroup[] } };
type Registration = {
  id?: string;
  name?: string;
  course?: string;
...
function normalizeCourseConfigSets(raw: unknown): CourseConfigSet[] {
  if (!raw) return [];
  if (Array.isArray(raw)) {
    return raw
      .map((p) => (typeof p === 'string' ? { name: p, data: null } : p))
      .filter((p): p is CourseConfigSet => Boolean(p && typeof p.name === 'string'))
      .map((p) => ({ name: p.name, data: p.data }));
  }
  if (typeof raw === 'object') {
    const record = raw as Record<string, CourseConfigSet['data']>;
    return Object.keys(record).map((name) => ({ name, data: record[name] }));
  }
  return [];
}

function extractCourseTreeFromCourseConfigSet(courseConfigSet?: CourseConfigSet | null) {
  return Array.isArray(courseConfigSet?.data?.courseTree) ? courseConfigSet.data.courseTree : [];
}

function extractCourseConfigSetCourses(courseConfigSet?: CourseConfigSet | null) {
  const out: string[] = [];
  extractCourseTreeFromCourseConfigSet(courseConfigSet).forEach((g) => (g.items || []).forEach((i) => {
    if (i.label) out.push(i.label);
  }));
  return Array.from(new Set(out.filter(Boolean)));
}

function buildCourseCategoryMap(courseTree: CourseTreeGroup[]) {
  const map = new Map<string, string>();
  courseTree.forEach((g) => (g.items || []).forEach((i) => {
...
const RegistrationsTab = () => {
  ...
  const courseConfigSetCategories = useMemo(() => {
    const categories = Array.from(
      new Set(courseConfigSetTree.map((g) => g.cat).filter(Boolean))
    ) as string[];
    return categories;
  }, [courseConfigSetTree]);
  ...
  async function loadRegistrations() {
    ...
    try {
      ...
      const results: Registration[] = res?.results || [];
      setRegistrations(results);
    } catch (e) {
      const message = e instanceof Error ? e.message : '등록현황을 불러오지 못했습니다.';
      setError(message);
      setRegistrations([]);
    } finally {
      setLoading(false);
  ...
  const filteredRegistrations = useMemo(() => {
    ...
    if (allowed && allowed.size) {
      list = list.filter((r) => {
        ...
        if (course && allowed.has(course)) return true;
        for (const base of allowed) {
          if (course && String(course).startsWith(base)) return true;
        }
        return false;
      });
    }
    ...
    if (courseFilter) {
      if (isMergeKey(courseFilter)) {
        ...
        const courses = mg?.courses || [];
        if (courses.length) {
          list = list.filter((r) => typeof r.course === 'string' && courses.includes(r.course));
        } else {
          list = [];
        }
      } else {
  ...
  async function persistMerges(next: MergeEntry[]) {
    try {
  ...
  async function deleteMerge(id: string) {
