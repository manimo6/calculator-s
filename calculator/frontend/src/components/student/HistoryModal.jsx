...
import { courseConfigSetName, courseTree } from '../../utils/data.js';
import {
...
const HistoryModal = ({ isOpen, onClose, onSelect, onEdit, editingId, calendarMinDate, calendarMaxDate }) => {
    ...
    const fetchHistory = async (pageNum, search = searchTerm) => {
        setLoading(true);
        try {
    ...
    const handleTransferSave = async () => {
        ...
        try {
            ...
            setTransferError(e.message || '전반 처리에 실패했습니다.');
        } finally {
    ...
    const renderHistoryList = (mode) => (
        <div className="space-y-2 max-h-[320px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-200">
            {historyData.map((item) => {
                ...
                const isEditing = editingId && String(editingId) === String(item.id);
