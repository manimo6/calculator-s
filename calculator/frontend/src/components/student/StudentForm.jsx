...
import { normalizeCourseConfigSets } from '../../features/admin/courseConfigSets/utils.js';
import CourseSelector from './CourseSelector.jsx';
import SingleCourseOptions from './SingleCourseOptions.jsx';
import CartList from './CartList.jsx';
import Modal from '../common/Modal.jsx';
import HistoryModal from './HistoryModal.jsx';
import { apiClient } from '../../api-client.js';
import CourseConfigSetPicker from '../../features/admin/courseConfigSets/CourseConfigSetPicker.jsx';
...
const readStoredCourseConfigSet = (scope) => {
...
const writeStoredCourseConfigSet = (scope, value) => {
    if (typeof window === "undefined") return;
    try {
...
function formReducer(state, action) {
    switch (action.type) {
        ...
        case 'UPDATE_SINGLE_INPUT':
            return {
                ...state,
                singleCourseInputs: { ...state.singleCourseInputs, [action.field]: action.value }
            };
...
const resolveCalendarRange = (settings) => {
...
const formatHistoryCourseSummary = (entry) => {
...
const resolveTextbookInfo = (item) => {
    if (!item) {
    ...
    const textbook = info.textbook || {};
    const courseType = item?.singleCourseInputs?.courseType;

    let textbookOption = textbook.defaultOption || 'none';
    let textbookAmount = Number(textbook.defaultAmount || 0);

    if (courseType === '온라인') {
        textbookOption = textbook.onlineOption || textbook.defaultOption || 'none';
        textbookAmount = Number(textbook.onlineAmount || textbook.defaultAmount || 0);
    } else if (courseType === '오프라인') {
        textbookOption = textbook.offlineOption || textbook.defaultOption || 'none';
...
const CourseConfigSetSidebar = ({
    ...
    const ActionButton = ({ icon: Icon, label, onClick, disabled: actionDisabled }) => (
        <button
            type="button"
            onClick={onClick}
...
const StudentForm = () => {
    ...
    const [calendarRange, setCalendarRange] = useState({ minDate: null, maxDate: null });
    ...
    const loadCourseConfigSets = useCallback(async (options = {}) => {
    ...
    const handleSelectCourseConfigSet = (name) => {
    ...
    const handleLoadHistory = (record) => {
        ...
        if (!foundKey) {
            foundKey = Object.keys(courseInfo).find(k => courseInfo[k].name === record.course);
        }

        if (!foundKey) {
            foundKey = Object.keys(courseInfo).find(k => record.course.startsWith(courseInfo[k].name));
        }
        ...
        dispatch({ type: 'SET_STUDENT_NAME', payload: record.name });

        const inputs = {
            startDate: record.startDate,
            period: parseInt(record.weeks) || 1,
            courseType: record.course.includes('온라인') ? '온라인' : (record.course.includes('오프라인') ? '오프라인' : ''),
            drwLevel: '',
            excludeMath: Boolean(record.excludeMath || record.course?.includes('수학제외')),
            recordingEnabled: record.recordingDates && record.recordingDates.length > 0,
            recordingDates: record.recordingDates || [],
                                ...
                                <div className="animate-in fade-in slide-in-from-top-4 duration-300">
                                    <div className="py-6 border-t border-border">
                                        <SingleCourseOptions
                                            ...
                                            calendarMinDate={calendarRange.minDate}
                                            calendarMaxDate={calendarRange.maxDate}
