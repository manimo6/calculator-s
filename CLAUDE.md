...
| Phase | 작업 | 상태 | 리스크 |
|-------|------|------|--------|
| 1 | 서버 생존성 (에러핸들러, 셧다운, 로깅) | **대기** | 없음 (순수 추가) |
| 2 | 입력값 검증 레이어 | **대기** | 낮음 (순수 추가) |
| 3 | WebSocket 보강 (Rate Limit, 인증 분리) | **대기** | 중간 (리팩토링) |
| 4 | Error Boundary (프론트) | **대기** | 없음 (순수 추가) |
| 5 | 에러 메시지 Sanitize | **대기** | 낮음 (동작 변경) |
| 6 | CORS 기본값 수정 | **대기** | 없음 (1줄) |
| 7 | CSRF 로직 수정 (프론트+백 동시) | **대기** | 높음 (2단계 롤아웃) |
